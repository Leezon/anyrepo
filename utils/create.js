...
class Creator {
  ...
  async getCollectRepo() {
    ...
    loading.start();
    const { data: list } = await getRepoList(this.options.url);
    loading.succeed();
    ...
    const templateUrl = list.find((item) => item.name === choiceTemplateName).full_name;
    this.downloadTemplate(templateUrl);
  }
